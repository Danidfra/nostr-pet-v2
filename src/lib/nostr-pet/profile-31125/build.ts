 ...
 */
export const buildBlobbonautProfileEvent = (profile: BlobbonautProfile): NostrEvent => {
  ...
  const event: NostrEvent = {
    id: '', // Will be generated by nostrify when publishing
    sig: '', // Will be generated by nostrify when publishing
    kind: BLOBBONAUT_PROFILE_KIND,
    content: '', // CRITICAL: Must always be empty string
    tags: finalTags,
    created_at: Math.floor(Date.now() / 1000),
    pubkey: profile.ownerPubkey,
  };
...
export const createBlobbonautProfile = (params: CreateBlobbonautProfileParams): BlobbonautProfile => {
  ...
  const profile: BlobbonautProfile = {
    event: {} as NostrEvent, // Will be created by build function
  ...
  // Build the event
  profile.event = buildBlobbonautProfileEvent(profile);
...
export const updateBlobbonautProfile = (
  ...
  updatedProfile.event = buildBlobbonautProfileEvent(updatedProfile);
